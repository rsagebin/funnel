...
class CategoriesViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        ...
        if pickerView == pickerOne {
            self.labelOne.text = self.category[row]
            self.mainLabel.text = "\(self.labelOne.text!)"
//            self.pickerOne.isHidden = true
        }
            
        else if pickerView == pickerTwo {
            self.labelTwo.text = self.subCategory[row]
            self.mainLabel.text = "\(self.labelOne.text!)/\(self.labelTwo.text!)"
//            self.pickerTwo.isHidden = true
        }
            
        else if pickerView == pickerThree {
            self.labelThree.text = self.subSubCategory[row]
            self.mainLabel.text = "\(self.labelOne.text!)/\(self.labelTwo.text!)/\(self.labelThree.text!)"
